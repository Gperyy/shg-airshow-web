...
const ShowProgram: React.FC<ShowProgramProps> = ({ lang }) => {
  ...
  const programs: Record<number, ProgramItem[]> = {
    0: [
      { time: '10:00 - 10:30', aircraft: lang === 'TR' ? 'F-16 "Solo Türk"' : 'F-16 "Solo Türk"', pilot: 'Yzb. Emre MERT', duration: lang === 'TR' ? '30 dk' : '30 min' },
      { time: '11:00 - 11:45', aircraft: lang === 'TR' ? '"Türk Yıldızları"' : '"Turkish Stars"', pilot: lang === 'TR' ? 'Akrotim' : 'Aerobatic Team', duration: lang === 'TR' ? '45 dk' : '45 min' },
      { time: '12:00 - 12:20', aircraft: 'P-51 Mustang', pilot: 'Ali İsmet ÖZTÜRK', duration: lang === 'TR' ? '20 dk' : '20 min' },
      { time: '13:00 - 13:30', aircraft: 'HÜRKUŞ', pilot: lang === 'TR' ? 'TUSAŞ Test Pilotları' : 'TUSAŞ Test Pilots', duration: lang === 'TR' ? '30 dk' : '30 min' },
      { time: '14:00 - 14:20', aircraft: lang === 'TR' ? 'ATAK Helikopteri' : 'ATAK Helicopter', pilot: lang === 'TR' ? 'Kara Havacılık Kom.' : 'Army Aviation Cmd.', duration: lang === 'TR' ? '20 dk' : '20 min' },
      { time: '15:00 - 15:30', aircraft: lang === 'TR' ? 'Jandarma Çelik Kanatlar' : 'Gendarmerie Steel Wings', pilot: lang === 'TR' ? 'Jandarma Akrotim' : 'Gendarmerie Aerobatics', duration: lang === 'TR' ? '30 dk' : '30 min' },
    ],
    1: [
      { time: '10:00 - 10:20', aircraft: 'T-6 Texan', pilot: lang === 'TR' ? 'SHM Ekibi' : 'SHM Crew', duration: lang === 'TR' ? '20 dk' : '20 min' },
      { time: '11:00 - 11:30', aircraft: 'HÜRJET', pilot: lang === 'TR' ? 'TUSAŞ Test Pilotu' : 'TUSAŞ Test Pilot', duration: lang === 'TR' ? '30 dk' : '30 min' },
      { time: '12:00 - 12:45', aircraft: lang === 'TR' ? '"Türk Yıldızları"' : '"Turkish Stars"', pilot: lang === 'TR' ? 'Akrotim' : 'Aerobatic Team', duration: lang === 'TR' ? '45 dk' : '45 min' },
      { time: '13:30 - 14:00', aircraft: 'F-16 "Solo Türk"', pilot: 'Yzb. Emre MERT', duration: lang === 'TR' ? '30 dk' : '30 min' },
      { time: '14:30 - 14:50', aircraft: 'Pitts S-2B', pilot: 'Ali İsmet ÖZTÜRK', duration: lang === 'TR' ? '20 dk' : '20 min' },
      { time: '15:30 - 16:00', aircraft: lang === 'TR' ? 'Final Geçişi' : 'Final Fly-by', pilot: lang === 'TR' ? 'Tüm Katılımcılar' : 'All Participants', duration: lang === 'TR' ? '30 dk' : '30 min' },
    ]
  };
  ...
  const translations = {
    ...
    tableHeader: {
      ...
      pilot: lang === 'TR' ? 'Pilot / Ekip' : 'Pilot / Crew',
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse table-fixed min-w-[600px]">
              <thead className="bg-gray-50 dark:bg-black/40">
                <tr>
                  <th className="p-5 md:p-8 font-extrabold text-[10px] md:text-xs text-gray-500 dark:text-gray-400 uppercase tracking-[0.2em] w-[25%]">{translations.tableHeader.time}</th>
                  <th className="p-5 md:p-8 font-extrabold text-[10px] md:text-xs text-gray-500 dark:text-gray-400 uppercase tracking-[0.2em] w-[35%]">{translations.tableHeader.aircraft}</th>
                  <th className="p-5 md:p-8 font-extrabold text-[10px] md:text-xs text-gray-500 dark:text-gray-400 uppercase tracking-[0.2em] w-[25%] hidden sm:table-cell">{translations.tableHeader.pilot}</th>
                  <th className="p-5 md:p-8 font-extrabold text-[10px] md:text-xs text-gray-500 dark:text-gray-400 uppercase tracking-[0.2em] w-[15%] hidden md:table-cell">{translations.tableHeader.duration}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {programs[activeDay].map((item, idx) => (
                  <tr key={idx} className="hover:bg-primary/[0.02] dark:hover:bg-primary/[0.05] transition-colors group">
                    ...
                    <td className="p-5 md:p-8 whitespace-nowrap text-gray-600 dark:text-gray-400 font-medium hidden sm:table-cell text-sm align-middle overflow-hidden text-ellipsis">
                      {item.pilot}
                    </td>
                    <td className="p-5 md:p-8 whitespace-nowrap text-gray-600 dark:text-gray-400 font-bold hidden md:table-cell text-sm align-middle">
                      {item.duration}
                    </td>
                  </tr>
                ))}
