...
const Tickets: React.FC<TicketsProps> = ({ lang }) => {
  ...
  const faqs = [
    {
      q: lang === 'TR' ? 'Biletleri iade edebilir miyim?' : 'Can I refund my tickets?',
      a: lang === 'TR' 
    ...
    {
      q: lang === 'TR' ? 'Etkinlik alanına yiyecek ve içecek getirebilir miyim?' : 'Can I bring food and drinks to the event area?',
      a: lang === 'TR'
        ? 'Güvenlik nedeniyle etkinlik alanına dışarıdan yiyecek ve içecek getirilmesi yasaktır. Alan içerisinde çok sayıda yiyecek ve içecek standı bulunmaktadır.'
        : 'For security reasons, it is prohibited to bring outside food and beverages into the event area. There are numerous food and beverage stands available inside the area.'
    },
    {
      q: lang === 'TR' ? 'Çocuklar için yaş sınırı var mı?' : 'Is there an age limit for children?',
      a: lang === 'TR'
        ? 'Etkinliğimiz her yaşa açıktır. 6 yaş ve altındaki çocuklar için giriş ücretsizdir. Aile Paketi, 7-14 yaş arası çocuklar için avantajlı bir seçenektir.'
        : 'Our event is open to all ages. Entry is free for children aged 6 and under. The Family Package is an advantageous option for children aged 7-14.'
    },
    {
      q: lang === 'TR' ? 'Otopark mevcut mu?' : 'Is parking available?',
      a: lang === 'TR'
        ? 'Evet, etkinlik alanı çevresinde ücretli otopark alanları mevcuttur. VIP bilet sahipleri için özel ve ücretsiz otopark alanı bulunmaktadır.'
        : 'Yes, paid parking areas are available around the event site. There is a private and free parking area for VIP ticket holders.'
    }
  ];
  ...
  return (
    <div className="font-display">
      ...
      <section className="relative py-32 lg:py-48 text-center text-white overflow-hidden rounded-2xl mb-16 min-h-[50vh] flex items-center justify-center">
        ...
        <div className="container mx-auto px-4 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 max-w-4xl mx-auto">
            {pricing.map((tier, idx) => (
              ...
              >
                <h3 className="text-3xl font-black mb-2 text-gray-900 dark:text-white uppercase tracking-tight group-hover:text-primary transition-colors">{tier.title}</h3>
                ...
                <ul className="space-y-5 mb-12 flex-grow">
                  {tier.features.map((feature, fIdx) => (
                    <li key={fIdx} className="flex items-start gap-4">
                      <span className="material-icons text-2xl text-primary group-hover:scale-110 transition-transform">check_circle</span>
                      <span className="text-gray-600 dark:text-gray-400 font-semibold leading-tight">{feature}</span>
                    </li>
                  ))}
                </ul>
      ...
      <section className="bg-gray-50 dark:bg-gray-900/30 py-24 rounded-[3rem] mb-16 border border-gray-100 dark:border-gray-800">
        <div className="container mx-auto px-4 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h2 className="text-4xl font-black mb-4 text-gray-900 dark:text-white tracking-tight uppercase">{translations.faqTitle}</h2>
            <p className="text-lg text-gray-600 dark:text-gray-400 font-bold opacity-70">{translations.faqSubtitle}</p>
          </div>
          <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
            {faqs.map((faq, idx) => (
              <div key={idx} className="bg-white dark:bg-gray-900/80 p-8 rounded-2xl border border-gray-100 dark:border-gray-800 hover:border-primary/20 transition-all shadow-sm">
                ...
                <p className="text-gray-600 dark:text-gray-400 text-sm font-medium leading-relaxed pl-5">{faq.a}</p>
              </div>
            ))}
          </div>
        </div>
