...
const Navbar: React.FC<NavbarProps> = ({
  ...
  const translations = {
    ...
    partners: lang === "TR" ? "ÇÖZÜM ORTAKLARI" : "PARTNERS", // Yeni çeviri
    ...
    <header className="relative z-50 container mx-auto px-4 md:px-6 lg:px-8">
      <nav className="flex justify-between items-center h-20 md:h-24">
        ...
        <div className="hidden lg:flex items-center space-x-6 xl:space-x-8">
          {navLinks.map((link) => (
            <button
              key={link.id} // Name yerine id kullanımı daha güvenlidir
              onClick={() => {
                link.action();
                setIsMenuOpen(false);
              }}
