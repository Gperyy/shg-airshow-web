...
interface NavbarProps {
  onNavigate: (view: 'home' | 'program' | 'participants' | 'tickets') => void;
  currentView: string;
  lang: Language;
  onToggleLang: () => void;
}

const Navbar: React.FC<NavbarProps> = ({ onNavigate, currentView, lang, onToggleLang }) => {
  ...
  const navLinks = [
    ...
    { name: translations.contact, action: () => {}, id: 'contact' },
  ];
      ...
      <nav className="flex justify-between items-center h-20 md:h-24">
        ...
        <div className="flex items-center space-x-4">
          <button 
            ...
            className="bg-primary text-white font-extrabold py-2.5 px-6 xl:px-8 rounded-md hover:bg-red-700 transition-all hover:shadow-lg hover:shadow-primary/20 hidden md:block uppercase text-[10px] tracking-widest whitespace-nowrap"
          >
            {translations.buyTicket}
          </button>
          <button 
            className="lg:hidden text-gray-700 dark:text-gray-300 p-2 focus:outline-none"
      ...
      {isMenuOpen && (
        <div className="lg:hidden absolute top-full left-0 right-0 py-6 bg-white dark:bg-gray-900 rounded-b-xl shadow-2xl animate-in fade-in slide-in-from-top-4 duration-200 z-50 border-t border-gray-100 dark:border-gray-800">
          <div className="flex flex-col space-y-5 px-8">
            ...
            <div className="flex justify-between items-center border-b border-gray-50 dark:border-gray-800 py-2">
               <span className="font-bold uppercase text-xs tracking-widest text-gray-500">{lang === 'TR' ? 'DİL SEÇENEĞİ' : 'LANGUAGE'}</span>
               <button 
                onClick={() => { onToggleLang(); setIsMenuOpen(false); }}
                className="font-extrabold text-primary uppercase text-sm tracking-widest"
               >
            ...
            <button 
              ...
              className="bg-primary text-white font-extrabold py-4 text-center rounded-md hover:bg-red-700 uppercase text-sm tracking-widest shadow-lg"
            >
              {translations.buyTicket}
            </button>
          </div>
